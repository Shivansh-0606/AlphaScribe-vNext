...
**Milestone:** Backend Engineering M1 (frozen) → M2 (implementation, Phase L ✅ complete) → M5 (EQ-3, ✅ CTO-approved) → M6 (Observability Hardening, ✅ CTO-approved)
...
| [26](26_M6_Observability_Architecture_Review.md) | M6 — Observability Architecture Review | 🟡 **Approved with Changes** | Retrospective verification of `19`–`25`, categorized Architecture/Operations/Implementation-Hygiene. **Confirmed via reproduction**: correlation IDs never reach emitted application logs (`CorrelationIdFilter` attached to the wrong object — a pre-existing M2 Phase 1 defect, not introduced by M6) — High. Two docstrings (`app/container.py`, `infrastructure/streaming/sse.py`) assert live-wiring states that are false — High. `/health/ready` doesn't verify Redis under `JOB_BACKEND=redis` — Medium, conditional fix only. Dashboards/alerting/expanded load testing reclassified as operational maturity, not architectural deficiency — Low. |
| [28](28_Post_M6_Roadmap_Reconciliation.md) | Post-M6 Engineering Roadmap Reconciliation | ✅ complete (research-only) | Reconciles current repo state against `17`'s ratified 5-milestone roadmap (M5→M6→M7→M8→M9) now that M5/M6 are both closed. **Next milestone: M7 — Streaming/Lifecycle Test Hardening** (T-6/7/10/11/13/14, zero dependencies, no ADR needed) — READY. Explicitly does not resurrect Phase 6/7's remaining architectural items (router split, Docker) since `17`'s own reconciliation already deprioritized them. Flags 2 real, still-open M6-adjacent gaps found by direct code inspection: correlation id still doesn't appear in log *output* (format string never got `%(correlation_id)s` even after A1's filter fix) and `push()` still never logs pipeline events (`O-2`) — both offered as optional same-pass bundling for M7, not required for it. Google Social Auth vs. `CLAUDE.md` remains the one unresolved document conflict, explicitly not a milestone candidate (needs a product/CTO ruling, not code). |
| [27](27_M6_Fast_Follow_Execution_Review.md) | M6 — Fast-Follow Execution Review | ✅ **Approved** | Verifies `26`'s A1/A2/A3 closure against current code; documents the additional metrics/tracing scope (`19` §5a). **New finding, not in `26`**: the job-level parent span `19` §5a added was always closed as successful — `except` branches handle failures internally, so the span's `__exit__` never sees an exception. Fixed via explicit `Status(StatusCode.ERROR, ...)` in each failure branch, both graphs; regression tests exercise the real `_run_pipeline`/`_run_explanation` functions, not a helper in isolation. Also: `retrieval_duration_seconds` now recorded on exception paths (was return-only), an explicit SSE mid-stream-cancellation regression test (real `body_iterator.aclose()`, not synthetic), and 4 accidental zero-byte root files removed. 180/180 hermetic+contract, 43/49 live (baseline-identical, zero regressions). Doc 26 itself is unmodified. |
| [29](29_M7_Streaming_Lifecycle_Test_Hardening_Implementation_Report.md) | M7 — Streaming/Lifecycle Test Hardening Implementation Report | 🟡 **ENGINEERING COMPLETE / EXTERNAL VERIFICATION PENDING** (not APPROVED, not CLOSED) | Closes the six test gaps `28` named (T-6 restart-recovery, T-7 PDF ingest, T-10 Redis keepalive, T-11 SSE reconnect, T-13 cancel-mid-stream event, T-14 strict event ordering) — 11 new/changed test functions, all real-path (not helper-in-isolation), zero production-code changes. 186/186 hermetic+contract passing; T-6/T-7/T-10/T-13 live-verified. T-11/T-14 are implemented and code-verified with live confirmation pending an adequately-provisioned normal-latency provider credential — three environments were tried (local Ollama, Gemini, NVIDIA NIM; report §§12–15), each blocked by a distinct provider/environment limitation rather than an application or test defect. The existing 300s job deadline (`LG-11`) was reviewed and explicitly kept unchanged (report §15). No further provider attempts are planned; the remaining requirement is a normal-latency, adequately provisioned provider credential (none specific mandated) for a later, separate verification-only task. **Governance:** implementation began before `28`'s required CTO pre-approval was formally recorded — a process violation, not a technical defect; the CTO explicitly granted a one-time retroactive exception during the review conversation on 2026-08-09 (report §6), and the standing approval-before-implementation rule is reaffirmed going forward (report §7). |
